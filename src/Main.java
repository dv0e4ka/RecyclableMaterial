...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        printMenu();
/*
Чтобы перевести введённую команду в тип ChatCommand,
используйте метод ChatCommand.valueOf(commandValue).
 */
        String commandValue = scanner.nextLine();
        ChatCommand command = ChatCommand.valueOf(commandValue);
        switch (command) {
            case MAP :
                System.out.println(MAP_LINK.toString());
                break;
            case RECYCLABILITY:
                ...
                break;
            case BONUS:
                System.out.println("Введите количество вторсырьяб кг:");
                int weight = scanner.nextInt();
                int bonus;
                if (weight < 10) bonus = weight * 10;
                else bonus = weight * 15;
    ...
    // добавьте модификаторы в заголовок метода
     static Map<MaterialType, String> getContainers() {
